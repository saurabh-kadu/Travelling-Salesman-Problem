...
num_cities = 10 # configure how many cities to populate the world with
world_x = 1000 # set the max x,y dimensions for the world
world_y = 1000
increment = 360.0/num_cities # calculate the increment required if we want to lay the cities out in a circle

aco = paco.ACO(num_cities,initial_pheromone=1,alpha=1,beta=3,epsilon=0,pheromone_deposit=2,evaporation_constant=0.6) # intialize the aco algorithm with our parameters
...
shortest_path = aco.get_best_path(num_ants=5,num_steps=10) # run the aco algorithm and return the shortest path
